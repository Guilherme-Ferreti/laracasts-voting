    ...
    <div class="flex items-center justify-center min-h-screen">
        ...
        >
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <p class="text-xs text-center leading-5 text-gray-500 px-6 mt-4">{{ __('You have one hour to edit your idea from the time you created it.') }}</p>

                <form wire:submit.prevent="updateIdea" action="#" method="POST" class="space-y-4 px-4 py-6">
                    <div>
                        <input wire:model.defer="title" x-ref="title" type="text" class="w-full text-sm bg-gray-100 border-none rounded-xl placeholder-gray-900 px-4 py-2" placeholder="{{ __('Your Idea') }}" required>
                        @error('title')
                    ...
                    @enderror
                    <div>
                        <textarea wire:model.defer="description" name="idea" id="idea" cols="30" rows="4" class="w-full bg-gray-100 rounded-xl border-none placeholder-gray-900 text-sm px-4 py-2" placeholder="{{ __('Describe your idea') }}" required></textarea>
                        @error('description')
                            <p class="text-red text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
